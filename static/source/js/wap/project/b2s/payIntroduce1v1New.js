define("payIntroduce1v1New",["fastclick","swiper-3.3.1.jquery.min"],function(require, exports, module) {
...
//未选择课程时弹窗
    $to_pay_on.on("click",function(){
        ...
        $to_pay_on_pop.show();
        $choose_pop.show();
        $class_to_buy.animate({bottom:0},500);
        //样式恢复
...
//已选课程弹层
    ...
    //弹层出现点击空白处消失
    function toHide(){
        $class_to_buy.animate({bottom:"-17rem"},1000);
        $choose_class_pop_c.animate({bottom:"-10rem"},1000);
        setTimeout(function(){
            $to_pay_on_pop.hide();
            $choose_pop.hide();
            $to_pay_on.show(); 
