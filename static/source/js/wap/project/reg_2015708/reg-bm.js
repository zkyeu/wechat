...
define(function(require,exports,module){
  ...
  $(document).ready(function() {
    ...
    //绘本馆弹层关闭
    $(".picture_close_btn").click(function() {
        $("#picture_mark").hide();
    });
    ...
    //美小
    if ($('#getPurpose').length) {
        if ($("#tabFormUSA").length > 0) {
            var occUp = $(".occup li .active");
            if (occUp.attr("data") == 6) {
                console.log("hi" + "6");
                $("input[name='grade']").val("");
                $('#mdSchoolUSA').show();
            }
            if (occUp.attr("data") == 4) {
                specialSituation = "special";
                $("input[name='purpose_desc']").val('');
                $("#checkGradeUSA").show();
                $("#checkPurposeUSA").hide();
            } else if (occUp.attr("data") == 7) {
                specialSituation = "special";
                $("input[name='purpose_desc']").val(77);
                $('#ageUSA').show();
                $("#checkPurposeUSA").hide();
