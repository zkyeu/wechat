...
define(function(require, exports, module) {
	...
	SubmitFn.prototype = {
		_event:function(){
			var that = this;
			this.classTime.val($('.current').attr('time-data'));
			this.nameLevel.text($('.current').attr('class-data'));
			this.timeId.find('span').on('click',function(){
				if(!$(this).hasClass('curren')){
					that.timeId.find('span').removeClass('current');
					$(this).addClass('current');
					that.classTime.val($(this).attr('time-data'));
					that.nameLevel.text($('.current').attr('class-data'));
				}
			});
			this.submitBtn.on('click',function(){
				that.formSubmit.submit();
			});
		}
	};
	new SubmitFn();
