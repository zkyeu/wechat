 ...
 */
define("groupBuying",[""],function(require,exports,module){
    //开团倒计时
    (function(){
      var hour1 = $('#hour1'),
        hour2 = $('#hour2'),
    ...
    function timer(intDiff) {
      ...
      function wxPromptFn(){
        var closeId = $('#closeId'),
            shareId = $('#shareId');
        if(localStorage.getItem('shareValue') == 'true'){
          shareId.hide();
        }else{
          shareId.show();
        }
        closeId.on('touchend',function(){
          localStorage.setItem('shareValue','true');
          shareId.hide();
        });
      }
      
      //判断是否是微信打开
      var wxua = navigator.userAgent.toLowerCase();//获取判断用的对象
