define(function(require,exports){
	...
	// 生成二维码
		// var $bg_code = $(".bg_code");
		// var $code_input = $(".bg_code").find("input");
		// var $img_url = $code_input.attr("code_url");
		// // alert($img_url);
		// var $AC_img = $("<img src="+$img_url+" />");
		// $bg_code.append($AC_img);
		var code_url = document.getElementById('code_url').value;
	    ...
	    var id_1v1 = $("#tran_data").attr("id_1v1"),
	        id_b2s = $("#tran_data").attr("id_b2s"),
	        url_data = $("#tran_data").attr("url_data"),
	        url_succ = $("#tran_data").attr("url_succ");
	    window.cnscan = setInterval(function(){
	      $.ajax({
	      	url: url_data,
	      	data : {
	      		id_1v1 : id_1v1,
	        ...
	        if(data=='1'){
	          window.location.href = url_succ+"?id_b2s="+id_b2s+"&id_1v1="+id_1v1;
	        }
	        var cn = $('#qrcode').data('count');
	        if(!cn){
	          cn = 1;
	        }
	        if(cn >150){
	          clearInterval(window.cnscan);
	        }
	        $('#qrcode').data('count',cn+1);
	      }})
	    },2000);
