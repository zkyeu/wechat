define('index',['cart'],function(require,exports){
    ...
    //领取奖学金
    !function(){
        $(".scholarModule").find(".defaultSta").find(".btn").on("click",function(){  console.log("成功点击");
            ...
            $.ajax({
                ...
                success: function(res){
                    if(res.status==1){ 
                        _this.parent(".list").removeClass("defaultSta");
                        $(".mask,.scholarship").show(); 
                        $(".scholarship").find(".price").html(res.data.money+"元");
                        _this.html("已<br>领 <br>取<i></i>").unbind("click"); 
                       return false;
                    }
                    if(res.status==0){
