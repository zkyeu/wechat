...
$(function(){
    ...
    $(function(){
       ...
       // $.getJSON("http://one.frontend.com/static/js/html/project/AC/lesson-memo.json",function(res){
           /* var datas = res.data;
            console.log(datas);
           var compiled = _.template(document.getElementById("dialog-w").innerHTML);
           console.log(compiled);
            $("#lesson-dialog").html(compiled({data:res.data}));*/
            function LessonMemo(){
                this.init();
            }
            LessonMemo.prototype={
                init : function(){
                    this.showDilogEvents();
                },

                bindEvents:function(){
                    //pronunciation
                    this.pronunciationEvents();
                    //gramlist
                    this.grammarEvents();
                    //Communication Skilll
                    this.skillEvents();
                    //list-level
                    this.levelListEvents();
                    // close lessonmemo dialog
                    this.closeEvents();

                    //
                    this.assessmentEvents();
                    //Choose Stage 
                    this.chooseStageEvents();
                    //提交
                    this.submitEvents();
                ...
                changeTextarea:function(){
                      $("#assessment_change").click(function(){
                        //globalBtn
                            $.ajax({
                                url:"http://tms.51talk.com/AcLessonMemo/changeGeneral",
                                type:"get",
                                //data:{occup:globalBtn.data("occup")},
                                data:{occup:1},
                                success:function(res){
                                   
                                    res= JSON.parse(res);
                                        if(res.data){
                                            $('#assessment1').val(res.data);
                                        }
                                        
                                }
                            })
                      }); 
                      $("#suggestion_change").click(function(){
                            $.ajax({
                                url:"http://tms.51talk.com/AcLessonMemo/changeGeneral",
                                type:"get",
                                data:{
                                    //occup:globalBtn.data("occup"),
                                    data:{occup:1},
                                    now_level:!globalBtn.data("stulevel")?0:globalBtn.data("stulevel")
                                },
                                success:function(res){
                                    res= JSON.parse(res);
                                    if(res.data){
                ...
                showDilogEvents:function(){
                    var self = this;
                    $(".memoflag,.levelflag").live("click",function(){
                        if($(this).hasClass("havenSubmit") || $(this).hasClass("check")){
                            return;
                        }

                        if($(this).hasClass("memoflag")){
                            var level = 0;
                        }else if($(this).hasClass("levelflag")){
                            var level=1;
                        }
                        window.globalBtn = $(this);
                        var data={
                            level:level,
                            tag_id:$(this).data("tageid"),
                            appoint_id:$(this).data("appointid"),
                            tea_user_id:$(this).data("teaid"),
                            status:"end"
                        }
                        $(".mask").show();
                        $("#lesson-dialog").show();
                        $.ajax({
                            url:"http://tms.51talk.com/AcLessonMemo/getLessonMo",
                            type:"get",
                            data:data,
                            success:function(res){
                                res = JSON.parse(res);
                ...
                pronunciationEvents:function(){
                    $(".box-bom").last().css({"border-bottom":"none","padding-bottom":"0","margin-bottom":"0"});
                    $("#selectedWords li").off().on("click",function(){
                    ...
                    $("#reviewList").on("click",'i',function(){
                        $(this).parent().remove();
                    });
                    $("#reviewList").on("click",'.com_addView',function(){
                        $(this).parent().before("<li><input name='soundmak' type='text' ><i></i></li>");
                    });
                },
                grammarEvents:function(){
                    $("#gramList").on("click",'.com_addView',function(){
                        $(this).parent().parent().before("<li class='com_dashTop'><input type='text' name='incorrect' class='error' ><input type='text' name='correct' class='succ' ><i></li>");
                         $("#gramList i").show();
                    });
                    $("#gramList").on("click","i",function(){
                        $(this).parent().remove();
                        if($("#gramList li").length==2){
                             $("#gramList i").hide();
                        }
                    });
                },
                skillEvents:function(){
                    $(".c-skill").each(function(){
                        var val = $(this).val();
                        if(val!=0){
                            $(this).parent().next().find("p").eq(val-1).show();
                        }
                        
                    });
                    $(".skill").on("click",'span',function(){
                        $(this).parent().find("span").removeClass('active');
                        var index = $(this).index()+1;
                        $(this).parent().prev().val(index);
                        $(this).parent().find("span").filter(":lt("+index+")").addClass('active');
                        $(this).parent().parent().next().find("p").eq(index-1).show().siblings().hide();
                    });
                    $(".skill").on("mouseover",'span',function(){
                        $(this).parent().find("span").removeClass('active');
                        var index = $(this).index()+1;
                        $(this).parent().find("span").filter(":lt("+index+")").addClass('active');
                        $(this).parent().parent().next().find("p").eq(index-1).show().siblings().hide();
                    });
                    $(".skill").on("mouseout",'span',function(){
                        $(this).parent().find("span").removeClass('active');
                        var index =  $(this).parent().prev().val();
                        $(this).parent().find("span").filter(":lt("+index+")").addClass('active');
                        if(index!=0){
                            $(this).parent().parent().next().find("p").eq(index-1).show().siblings().hide();
                        }else{
                ...
                levelListEvents:function(){
                    ...
                    $(".choose-level .title").on("click","a",function(){
                        var href = $(this).attr("href");
                        js2c('cpp_OpenURL',href);
                    });
                },
                assessmentEvents:function(){
                    $(".assessment-list").on("click","li",function(){
                        if(!$(this).find('input').is(":checked")){
                            $(this).find('input').attr("checked","checked");
                            $(this).find('i').addClass("active");
                            $(this).siblings().find("input").removeAttr("checked");
                            $(this).siblings().find("i").removeClass("active");
                            $("#edes-list").find("p").eq($(this).index()).show().siblings().hide();
                        }
                    });
                    $(".assessment-list li").each(function(){
                        if($(this).find("input").is(":checked")){
                            $("#edes-list").find("p").eq($(this).index()).show().siblings().hide();
                        }
                    });
                ...
                closeEvents:function(){
                    $(".h-title").on("click",".close",function(){
                        $("#lesson-dialog").hide();
                        $(".mask").hide();
                    });
                },
                chooseStageEvents:function(){
                    $(".studying-box").click(function(){
                        var input =$(this).find("input");
                        if(!input.is(":checked")){
                            input.attr("checked","checked");
                            $(this).find("i").addClass("active");
                            input.parent().parent().siblings().find("input").removeAttr("checked");
                            input.parent().parent().siblings().find("i").removeClass("active");
                        }
                        var subvel = $(this).data("json");
                         var compiled = _.template(document.getElementById("dialog-level").innerHTML);
                        $("#subLevel").html(compiled({sublevel:subvel}));
                ...
                submitEvents : function(){
                   ...
                   $("#submit").on("click",function(){
                        var incorrect =  $("[name='incorrect'][value='']").length>0?false:true;
                       var correct = $("[name='correct'][value='']").length>0?false:true;
                       var skill = $(".c-skill[value=0]").length>0?false:true;
                       if( $("[name='note']").length>0){
                            var note =  $("[name='note']").val().length==0?false:true;
                       }else{
                            var note = true;
                       }
                       
                       if($("[name='assessment']").length>0){
                                var assessment = $("[name='assessment']").val().length==0?false:true;
                       }else{
                                 var assessment = true;
                       }
                       if($("[name='suggestion']").length>0){
                             var suggestion = $("[name='suggestion']").val().length==0?false:true;
                       }else{
                             var suggestion = true;
                       }
                  
                        var keyWord = $("[name='key-word']:checked").length==0?false:true;
                        var soundmak = $("[name='soundmak'][value='']").length>0?false:true;    
                        if($(this).text()=="sending"){
                            return;
                        }
                        $("#submit").text("sending...");
                        if(incorrect && correct && skill && note && assessment && suggestion && keyWord && soundmak){
                            ...
                            $.ajax({
                                ...
                                data:data,
                                success:function(){
                                     globalBtn.addClass("havenSubmit");
                                     var appointid = globalBtn.data("appointid");
                                     var selectList = $("#myComplete [data-appointid='"+appointid+"']");
                                     var submitList = $("#myComplete [data-appointid='"+appointid+"']").filter("havenSubmit");
                                     if(selectList.length==selectList.length){
                                            nosubTotal= nosubTotal-1;
                                     }    
                                     $("#pass-list li").eq(1).find(".noSubmit").text(nosubTotal);
                                     if(nosubTotal>0){
                                    $('#pass-list li').eq(1).find(".noSubmit").show().text(nosubTotal);
                                     }else{
                                            $('#pass-list li').eq(1).find(".noSubmit").hide();
