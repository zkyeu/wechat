define("heckCircle",["plupload","niceScroll","fancybox"],function(require,exports,module){
    ...
    //展示转发朋友圈层
    $('.js-broad-mes').on('click', function(e){
        zfLayer.show();
        zfLayer.find('.confirm').attr("data-id",$(this).attr("data-admin-id"));
        var content = $(this).attr("data-share-words");
        zfLayer.find('textarea').val(content);
    });

    //确定转发
    zfLayer.find('.confirm').on('click', function(){
        var content = zfLayer.find('textarea').val();
        var id = zfLayer.find('.confirm').attr("data-id");
        $.ajax({
            url:'/CircleFriend/transmitCircleFriend',
            dataType : "json",
            data:{
                wechat_mid: id,
                share_word:content
            ...
            success:function(data){
                ...
                console.log(data.status !=10000);
                if(data.status != 10000) return alert(data.message);
                $('.zhuanfa-layer').attr("style","display:none");
                console.log("hide ok")
            },
            error:function(data){
