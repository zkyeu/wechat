 ...
 * @version 1.0.0
 */
define("class", ["lodash", "tmpl","videos"], function(require, exports, module) {
    var tmpl = require('tmpl');
    var domain2 = document.domain;
    var cookie = require("cookie");
    var dataIndex = -1; //播放区分标志
    var page = $("#page").val(); //页码
    var course = $("#course").val() || 0;
    ...
    var tag = $("#tag").val();
    var recordTime = 0;
    var is_guide = $("#course_tag").val();
    var cur = 1;
    var jumpFlag = 0;
    var beta_tag = $('#beta_tag').val(); //0 显示选内测版弹出框 显示选项卡(弹窗 使用体验版、原版)使用
    var is_beta = $('#is_beta').val(); //该课程是否是体验版课程 都是1 为真，0 为显示弹窗及不是体验课程
    var timeId;
    var timer;
    window.student_num = $("#student_num").val();
    var startTime; //进入页面时间
    var endTime;
    var requestStart; //进入开始响应时间
    var requestEnd; //进入结束响应时间
    var talk_tag = parseInt($("#talk_tag").val()); //0 时显示对话框显示  1时不显示对话框
    //var talk_tag =0;
    var runoff_tag = parseInt($("#off_tag").val()); //0时显示流失弹出框 1时不显示弹出框对话框
    var talk_id=1;
    //var runoff_tag = 0;
    var option = {
        course_id: course,
    ...
    var model_tag= $("#model_tag").val().replace(/,/g,"");
    var xinaoFlage=null;
            ...
            if (res == 2 && is_guide == "0") {
                setTimeout(function() {
                    ...
                    $(".w3container .btn").click(function() {
                        ...
                        is_guide = 1;
                        $.ajax({
                            url: "/ajax/addPreClassLog?act=info",
                            type: "post",
                            data: {
                                key: 'course',
                                val: course
    ...
    function statistics(isKnow) {
        ...
        $.ajax({
            url: "/ajax/addPreClassLog?act=words",
            type: "post",
            data: data
        });
        startTime = endTime;
    ...
    function actCourse(type) {
        ...
        $.ajax({
            url: "/ajax/addPreClassLog?act=course",
            type: "post",
            data: postdata
        });
    }
    ...
    function nextBlind(isKnow) {
        ...
        $(".loading-pic").show();
        $.ajax({
            url: "/ajax/getpreclass?course=" + course + "&page=" + page + "",
            dataType: "json",
    ...
    function nextBind() {
        ...
        $("#next").unbind().click(function() {
            ...
            actCourse();
            var t_startTime =  new Date().getTime();
            $.ajax({
                url: "/ajax/getpreclass?course=" + course + "&page=" + page + "",
                dataType: "json",
                success: function(res) {
                    ...
                    if(switch1){
                         $.ajax({
                          ...
                          type: "post",
                          data:{
                            url:"http://"+location.host+"/ajax/getpreclass?course=" + course + "&page=" + page + "",
                            type:"3",
                        ...
                        console.log("接口时间:"+(t_endTime - t_startTime))
    ...
    function survey() {
        ...
        if (tag == 0) {
            ...
            $("#selectTimeDialog .layerPoi span").click(function() {
                ...
                $.ajax({
                    url: '/ajax/addPreClassLog?act=user',
                    type: "post",
                    data: {
                        "tag": tagVal
                    }
...
function renderTalk(talkList){
    ...
    function initfirstPage(obj,defferd){
        var width=null;
        if(JSON.parse(tab_list).length==2){
            width = '50%';
        }else if(JSON.parse(tab_list).length==3){
            width = '33.333%';
        }else{
            width = '25%';
        }
        obj.html(tmpl.init({
                ...
                'student_num':student_num1,
                 'data':JSON.parse(tab_list),
                 'width':width
            }));

        if($(".list li[data-tab=2]").attr("model")==2){
            $("#step2").hide();
        }else{
            process(videoPage,videoTotal);
        }
        
            $(".under li").hover(function(){
                $(this).find(".tips").show();
                $(this).find(".tips").css("color","#fff");
            },function(){
                $(this).find(".tips").hide();
            })
                ...
                switch ($(this).attr("data-tab")) {
                    case "1":
                        ...
                        $.ajax({
                            ...
                            success: function(res) {
                                ...
                                if (res.type == 11 || res.type == 10 || res.type == 12 || res.type == 9) {
                                    $(".ft").hide();
                                } else {
                                    $(".ft").show();
                                }
                                apiInfoChoice(res).done(function() {
                                    ...
                                    word_idNew = res.id ? res.id : 0;
                                    is_end = res.is_end ? res.is_end : 0;
                                    if (is_beta == 1 && (res.type == 3 || res.type == 2)) {
                                        floatDialog(res);
                                    }
                                });
                    ...
                    case "2":
                        if($(this).attr('model')==2){
                                ...
                                $.ajax({
                                    url: "/ajax/getPreClassWord?course=" + course + "&type=dialogs",
                                    dataType: "json",
                                    success: function(res) {
                                        $(".loading-pic").hide();
                    ...
                    case "3":
                        ...
                        cur = 1;
                        $.ajax({
                            url: "/ajax/getPreClassWord?course=" + course + "&type=words",
                            dataType: "json",
                            success: function(res) {
                                $(".loading-pic").hide();
                                note($(".sub-class"), res);
                    ...
                    case "4":
                        ...
                        cur = 1;
                        $.ajax({
                            url: "/ajax/getPreClassWord?course=" + course + "&type=syntax",
                            dataType: "json",
                            success: function(res) {
                                $(".loading-pic").hide();
    ...
    function floatDialog(res) {
        $(".user-tip").show();
        if ((res.type == 3 || res.type == 2) && is_beta == 1) {
            $('.ft').hide();
            if (res.type == 2) {
            ...
            $(".float-box .pl-img").off().click(function() {
                ...
                $.ajax({
                    url: "/ajax/addPreClassLog?act=beta",
                    type: "post",
                    data: data
                });
                $("#next").trigger("click");
        ...
        } else if (res.type == 10 || res.type == 11 || res.type == 12) {
            $('.ft').hide();
            $(".user-tip").hide();
        } else {
            $(".float-box").hide();
    ...
    function scorePage(data) {
        ...
        $(".scoreBtn").click(function() {
            Init($(".wt-main")).done(function() {
                ...
                nextUnbind();
                $.ajax({
                    url: "/ajax/getpreclass?course=" + course + "&page=" + fPage + "",
                    dataType: "json",
                    success: function(res) {
                        $(".loading-pic").hide();
                        apiInfoChoice(res).done(function() {
                            ...
                            if (is_beta == 1) {
                                floatDialog(res);
                            } else if (model == 10 || model == 11 || model == 12) {
                                $(".user-tip").hide();
                                $(".ft").hide();
                            } else {
                                $(".user-tip").hide();
    ...
    function prePageEvent() {
        ...
        $("#firstPageDialog #confirm").off().click(function() {
            ...
            page = 0;
            $.ajax({
                url: "/ajax/getpreclass?course=" + course + "&page=" + 0 + "",
                dataType: "json",
                success: function(res) {
                    $(".loading-pic").hide();
...
function showScoreGuide(score){
   ...
   function  scoreGuide(){
    ...
    function guide(staytime,model){
         return $.ajax({
                url: "/ajax/addPreClassLog?act=notice",
                type: "post",
                data: {
                    model: model,
                    stay_time: staytime
        ...
        setTimeout(function() {
            ...
            $("#start_stu").click(function() {
                ...
                Init($(".wt-main")).done(function() {
                    ...
                    var courseId = location.href.match(/\/\d+/)[0].substring(1);
                    $('.helper a').each(function(){
                        var href = $(this).attr('href');
                        $(this).attr('href',href+'?courseid='+courseId);
                    });
